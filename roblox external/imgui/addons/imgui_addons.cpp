...
void ImAdd::Separator(const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    PushStyleColor(ImGuiCol_Text, ImVec4(255,255,255,255));
    TextV(fmt, args);
    PopStyleColor();
    va_end(args);
    ImGui::Separator();
...
bool ImAdd::Button(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (style.Colors[ImGuiCol_Button] != ImVec4(0, 0, 0, 0)) {
       // window->DrawList->AddRectFilledMultiColor(bb.Min + ImVec2(style.FrameBorderSize, style.FrameBorderSize), bb.Max - ImVec2(style.FrameBorderSize, style.FrameBorderSize),
         //   GetColorU32(style.Colors[ImGuiCol_Button]),
          //  GetColorU32(style.Colors[ImGuiCol_Button]),
           // GetColorU32(ImVec4(ImColor(8,8,8))),
           // GetColorU32(ImVec4(ImColor(8, 8, 8))));
    }
...
bool ImAdd::CheckBox(const char* label, bool* v)
{
    ...
    ImVec2 text_pos = pos + ImVec2(height + style.ItemInnerSpacing.x * 0.9f, (height - g.FontSize * 0.9f) * 0.5f);

    ImFont* old_font = g.Font;
    float old_size = g.FontSize;

    ImFont* font = g.Font;
    ImFont scaled_font = *font;
    scaled_font.Scale = 0.9f;
    g.Font = &scaled_font;
    g.FontSize = old_size * 0.9f;

    for (int x = -1; x <= 1; x += 2) {
        for (int y = -1; y <= 1; y += 2) {
            window->DrawList->AddText(g.Font, g.FontSize, text_pos + ImVec2(x, y), GetColorU32(ImGuiCol_TitleBg), label);
        }
    }
    window->DrawList->AddText(g.Font, g.FontSize, text_pos, GetColorU32(ImGuiCol_Text), label);
...
bool ImAdd::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, float width, const char* format)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    float slider_height = g.FontSize * 0.8f;
    float label_height = label_size.x > 0 ? g.FontSize + style.ItemInnerSpacing.y : 0.0f;
    const ImRect frame_bb(pos + ImVec2(0, label_height), pos + ImVec2(w, slider_height + label_height));
    const ImRect total_bb(pos, pos + ImVec2(w, slider_height + label_height));
    ...
    *((float*)p_data) = current_value[id];
    ...
    float t = (*(float*)p_data - *(float*)p_min) / (*(float*)p_max - *(float*)p_min);
    ImVec4 colA = GetStyleColorVec4(ImGuiCol_SliderGrab);
    ImVec4 colB = GetStyleColorVec4(ImGuiCol_SliderGrabActive);
    ImU32 fill_col = GetColorU32(ImLerp(colA, colB, t));

    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Min.x + t * frame_bb.GetWidth(), frame_bb.Max.y), fill_col, style.FrameRounding);
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(colFrame), style.FrameRounding, 0, style.FrameBorderSize);

    if (grab_bb.Max.x > grab_bb.Min.x) {
        window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(colGrab), style.GrabRounding);
    }
...
bool ImAdd::Selectable(const char* label, bool selected, const ImVec2& size_arg)
{
    ...
    if (selected) {
    // window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max, GetColorU32(ImAdd::HexToColorVec4(0x9acd32, 1)),
        // GetColorU32(ImAdd::HexToColorVec4(0x9acd32, 1)),
         //GetColorU32(ImAdd::HexToColorVec4(0x789e29, 1)),
        // GetColorU32(ImAdd::HexToColorVec4(0x789e29, 1)));
    }
...
bool ImAdd::ColorEdit3(const char* label, float* const col)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImAdd::SubTabButton(const char* label, const ImVec2& size_arg, bool fill, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImVec4 colFrame;
    if (fill)
        colFrame = GetStyleColorVec4((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    else
        colFrame = (held && hovered) ? GetStyleColorVec4(ImGuiCol_ButtonActive) :
        hovered ? GetStyleColorVec4(ImGuiCol_ButtonHovered) :
        ImVec4(0, 0, 0, 0);
...
bool ImAdd::TabButton(const char* label, const ImVec2& size_arg, bool fill, int flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImVec2 size = CalcItemSize(size_arg, label_size.x + style.FramePadding.x * 4.0f, label_size.y + style.FramePadding.y * 1.5f);
    ...
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, GetColorU32(it_anim->second.Frame), false, style.FrameRounding);
    if (fill) {
        window->DrawList->AddRectFilledMultiColor(bb.Min + ImVec2(style.FrameBorderSize, style.FrameBorderSize), bb.Max - ImVec2(style.FrameBorderSize, style.FrameBorderSize),
            GetColorU32(ImVec4(style.Colors[ImGuiCol_SliderGrab].x, style.Colors[ImGuiCol_SliderGrab].y, style.Colors[ImGuiCol_SliderGrab].z, 0.25)),
            GetColorU32(ImVec4(style.Colors[ImGuiCol_SliderGrab].x, style.Colors[ImGuiCol_SliderGrab].y, style.Colors[ImGuiCol_SliderGrab].z, 0)),
            GetColorU32(ImVec4(style.Colors[ImGuiCol_SliderGrab].x, style.Colors[ImGuiCol_SliderGrab].y, style.Colors[ImGuiCol_SliderGrab].z, 0)),
            GetColorU32(ImVec4(style.Colors[ImGuiCol_SliderGrab].x, style.Colors[ImGuiCol_SliderGrab].y, style.Colors[ImGuiCol_SliderGrab].z, 0)));
    }
