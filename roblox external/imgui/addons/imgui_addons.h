...
namespace ImAdd
{
    ...
    bool    TabButton(const char* label, const ImVec2& size_arg, bool fill, int flags = 0);
    bool    SubTabButton(const char* label, const ImVec2& size_arg, bool fill, ImGuiButtonFlags flags = 0);
    ...
    void    BeginChildCustom(const char* label, ImVec2 size = ImVec2(0, 0));
    void    EndChildCustom();

	bool	KeyBind(const char* str_id, int* k, float custom_width = 0);
	bool    ColorEdit4(const char* label, float col[4]);
	bool    ColorEdit3(const char* label, float* const col);
}
